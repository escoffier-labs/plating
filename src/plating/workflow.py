...
def render_workflow(data: dict) -> str:
    ...
    out = [
        ...
        f'  <text x="72" y="67" fill="{ACCENT}" font-family="IBM Plex Mono, ui-monospace, monospace" font-size="11" font-weight="600" letter-spacing="2.4">{_text(data["eyebrow"].upper())}</text>',
        f'  <text x="72" y="96" fill="{TEXT}" font-family="Inter, ui-sans-serif, sans-serif" font-size="21" font-weight="700" letter-spacing="-0.6">{_text(data["title"])}</text>',
        f'  <text x="72" y="119" fill="{MUTED}" font-family="IBM Plex Mono, ui-monospace, monospace" font-size="11">{_text(data["description"])}</text>',
    ]
    if data.get("meta"):
        out.append(
            f'  <text x="888" y="67" text-anchor="end" fill="{DIM}" font-family="IBM Plex Mono, ui-monospace, monospace" font-size="10">{_text(data["meta"])}</text>'
        )
    ...
    for column_index, column in enumerate(columns):
        x = left + column_index * (column_width + gutter)
        out.append(
            f'  <text x="{x:.1f}" y="164" fill="{ACCENT}" font-family="IBM Plex Mono, ui-monospace, monospace" font-size="10" font-weight="600" letter-spacing="1.7">{_text(column["title"].upper())}</text>'
        )
    ...
    for edge in data.get("edges", []):
        ...
        if edge.get("label"):
            ...
            label_y = (y1 + y2) / 2 - 8
            out.append(
                f'  <text x="{label_x:.1f}" y="{label_y:.1f}" text-anchor="middle" fill="{DIM}" font-family="IBM Plex Mono, ui-monospace, monospace" font-size="9">{_text(edge["label"])}</text>'
            )
    ...
    if context:
        out.extend(
            [
                ...
                f'  <text x="72" y="427" fill="{ACCENT}" font-family="IBM Plex Mono, ui-monospace, monospace" font-size="10" font-weight="600" letter-spacing="1.6">{_text(context["title"].upper())}</text>',
                f'  <text x="72" y="455" fill="{TEXT}" font-family="IBM Plex Mono, ui-monospace, monospace" font-size="13" font-weight="600">{_text(context["body"])}</text>',
            ]
        )
        if context.get("detail"):
            out.append(
                f'  <text x="72" y="478" fill="{DIM}" font-family="IBM Plex Mono, ui-monospace, monospace" font-size="10">{_text(context["detail"])}</text>'
            )
